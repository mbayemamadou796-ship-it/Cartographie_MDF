...
import { 
  ...
  UploadCloud, File as FileIcon, FileSpreadsheet, Image as ImageIcon, Download
} from 'lucide-react';
...
import { saveAttachmentToDB, downloadAttachment } from '../../utils/attachmentStorage';
...
export const ReferentReportingForm: React.FC<ReferentReportingFormProps> = ({
  ...
  const [formPreviewAttachment, setFormPreviewAttachment] = useState<ReportAttachment | null>(null);
  ...
  return (
    <div className="space-y-6">
                      ...
                      {piecesJointes.map((doc) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              type="button"
                              onClick={() => setFormPreviewAttachment(doc)}
                              title="Aperçu du document"
                              className="p-1.5 text-purple-700 hover:text-purple-950 rounded-lg hover:bg-purple-200/60 transition-all cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-600 hover:text-slate-950 rounded-lg hover:bg-slate-200/60 transition-all cursor-pointer"
                            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredMyReports.map((r) => (
                  ...
                  >
                    ...
                    {/* Pieces jointes in history card */}
                      ...
                      <div className="bg-purple-50/70 border border-purple-200/80 rounded-xl p-2.5 space-y-1">
                        <div className="flex items-center justify-between text-xs text-purple-950 font-bold">
                          <span className="flex items-center gap-1.5">
                            ...
                            <span>{r.piecesJointes.length} pièce(s) jointe(s) :</span>
                          </span>
                          <span className="text-[10px] text-purple-700 font-semibold">Cliquer pour ouvrir</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 pt-0.5">
                          {r.piecesJointes.map((doc, idx) => (
                            <button
                              key={doc.id || idx}
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                setFormPreviewAttachment(doc);
                              }}
                              className="px-2 py-0.5 bg-white hover:bg-purple-100 text-purple-900 border border-purple-300 rounded-lg text-[11px] font-bold transition flex items-center gap-1 shadow-2xs cursor-pointer"
                            >
      ...
      {/* Referent Attachment Preview Modal */}
      <AttachmentPreviewModal
        isOpen={!!formPreviewAttachment}
        attachment={formPreviewAttachment}
        onClose={() => setFormPreviewAttachment(null)}
      />
